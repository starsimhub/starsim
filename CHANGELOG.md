...
- Two resolution primitives are provided; a simulation uses one scheme (`split` *or* `spawn_fine`) with a single `ratio`:
...
- Both primitives copy all module states and assign CRN-safe slots derived from the parent (independent but reproducible trajectories). The slot scheme is **recurrence-safe**: a coarse parent may be re-resolved repeatedly, each new fine cohort getting a disjoint reserved sub-block while prior descendants are alive and recycling a sub-block once its descendants die (up to `MAX_LIVE_COHORTS` concurrent live cohorts per parent). Re-splitting a fine sub-agent is unsupported and raises.
...
- `Births`, `Deaths`, and `Pregnancy` operate on whole bodies (`epi_weight`): fine sub-agents are excluded from birth/death/conception selection, counts/rates are body-weighted, and newborns inherit the mother's `epi_weight`. Networks exclude fine sub-agents from transmission, so a split parent keeps transmitting.
