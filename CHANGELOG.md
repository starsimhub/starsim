...
- *Regression/bugfix*: `ssl.Measles` called `ss.SIR.set_prognoses()`, which set `infected = True` without clearing it, so exposed agents were simultaneously infectious: `dur_exp` had no effect on transmission and the model behaved as an SIR model. It also sampled `dur_inf` and `p_death` twice per infection. Both are fixed.
...
- *Regression/bugfix*: `ssl.Cholera.asymptomatic` excludes exposed agents, which do not shed. It is now defined via `define_aliases()` rather than as a property.
...
- Any keyword arguments to `define_states()` are passed to `define_aliases()`, so aliases can be declared alongside the states they refer to.
