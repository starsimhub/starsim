...
- `Module.define_states(reset=...)` now accepts a state or alias name (or list of names) as well as `True`, so a subclass can replace or remove individual inherited states and aliases rather than all of them. Removing a name without redefining it leaves it undefined, which is how `ss.SEIR` drops `ti_infected`. *Migration*: `reset=True` now clears inherited aliases as well as states, so `ss.SIR` re-declares `infectious` explicitly.
- `define_aliases()` now raises if an alias would be shadowed by an existing state of the same name, rather than silently defining an alias that is never consulted.
