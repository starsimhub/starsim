...
### State aliases and derived states
- Added `Module.define_aliases()`, which defines a state name that resolves to something else. An alias is either the name of another state (e.g. `self.define_aliases(infectious='infected')`) or a callable taking the module as its only argument (e.g. `self.define_aliases(exposed=lambda self: self.infected & ~self.infectious)`). Any keyword arguments to `define_states()` are forwarded to it, so aliases can be declared alongside the states they refer to.
- Aliases exist so a base class can supply a default that a subclass can replace. Unlike states, which cannot be redefined without `define_states(..., reset=True)`, an alias may replace an existing state or alias of the same name, so a subclass can derive a state that its parent stored directly.
- A callable alias is a *derived state*: it generates an `n_<name>` result like a real state, but has no storage of its own, does not appear in `state_list`/`state_dict`, and is recomputed on each access. Derived values are returned read-only, and writing to one raises an error.
- *Migration*: `ss.Infection.infectious` is no longer a property; it is now an alias of `infected`, declared in `define_states()`. Reading `disease.infectious` is unchanged, but a subclass that previously overrode the property (e.g. `def infectious(self): return self.infected | self.exposed`) should instead declare `infectious` as a state in its own right, which requires no override and no `super()` call.
- `Module` now defines `__getattr__` to resolve aliases. As part of this, missing-attribute errors list the module's available states, aliases and other attributes, and suggest the closest match.

### Infected vs. infectious
Starsim distinguishes `infected` ("has the infection") from `infectious` ("can transmit"). These coincide for SIR-type models, but differ for anything with a latent period, where an exposed agent is infected but not yet infectious. `new_infections` counts agents entering `infected`, so for these models `infected` must span both the exposed and infectious compartments, and `ti_infected` should be the time of the transmission event rather than the later onset of infectiousness.

- *Regression/bugfix*: `ssl.Measles`, `ssl.Cholera` and `ssl.Ebola` treated `infected` as the infectious compartment alone and set `ti_infected` to the onset of infectiousness. Because seed infections are excluded from `new_infections` on the first timestep, this produced a negative first timestep equal to minus the number of seed infections (e.g. `new_infections[0] = -196` for measles seeded with 196 initial cases), and `cum_infections` started below zero. All three now keep `infected` spanning the whole infected period (as in `ss.SIR`), add `infectious` as a state that starts after the latent period, and derive `exposed = infected & ~infectious`, so `new_infections` counts infection events and is never negative (issue [1389](https://github.com/starsimhub/starsim/issues/1389)).
- *Regression*: for those three diseases, `n_infected` (and hence `prevalence` and `cum_infections`) now reports E+I rather than I alone, and they gain an `n_infectious` result. `ti_infected` is now the time of infection; the onset of infectiousness is available as the new `ti_infectious`. `ti_exposed` is retained as an alias of `ti_infected`, since exposure and infection are the same event.
- *Regression/bugfix*: `ssl.Measles` called `ss.SIR.set_prognoses()`, which set `infected = True` without clearing it, so exposed agents were simultaneously infectious: `dur_exp` had no effect on transmission and the model behaved as an SIR model. It also caused `dur_inf` and `p_death` to be sampled twice per infection. Both are fixed.
- *Regression*: `ssl.Cholera.asymptomatic` is now `infectious & ~symptomatic` rather than `infected & ~symptomatic`, so it no longer counts exposed agents (which do not shed); it is also now a derived state, adding an `n_asymptomatic` result.
- *Migration*: models with a latent period should push `ti_recovered` and `ti_dead` back by the latent period, otherwise it eats into the infectious period instead of delaying it. Since `infected` keeps the meaning it has in `ss.SIR`, such a model can otherwise inherit `set_prognoses()` and most of `step_state()` unchanged.
- The SEIR examples in the tutorials and user guide have been updated accordingly, and the [Diseases](https://docs.starsim.org/user_guide/modules_diseases.html) user guide has a new section on the infected/infectious distinction and derived states.
