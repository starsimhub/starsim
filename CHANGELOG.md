...
- **Distribution RNG jumping is skipped under `ss.options.crn = False`.** With common random numbers disabled, the RNG stream advances naturally on each draw, so the per-step (`Module.start_step`) and per-draw (`Dist.rvs`) "jumps" that reseed each generator were pure overhead. They are now gated on `crn`: a single guard in `Dist.jump` neutralizes every jump path (the auto-jumps in `rvs`, `start_step`, and the manual jumps in `starsim/networks.py` and `multi_random`), and `start_step` skips the per-distribution jump loop entirely. The per-draw PCG64 state copy (`make_history`) is likewise skipped when it cannot be used. This speeds up `crn=False` runs by ~1.25–1.3x on distribution-heavy models. The default `crn=True` path is unchanged and bit-identical (`starsim/distributions.py`, `starsim/modules.py`).
...
- **Data columns are now matched against known synonyms.** `ss.Births` expected its birth-rate data to have a `Year` column, while `ss.Deaths` and `ss.Pregnancy` expected `Time` (the UN World Population Prospects label), so UN-native birth data required a `metadata=dict(data_cols=...)` override. `ss.standardize_data` now falls back to a small explicit synonym table (`ss.col_synonyms`) when the configured column is absent, so all three modules accept either spelling with no override; explicitly configured column names always take precedence. A genuinely missing column now raises a `ValueError` naming the expected column, listing the columns present, and pointing at the `metadata` override, rather than a bare `KeyError` (`starsim/utils.py`, `starsim/demographics.py`).
- Removed an unused branch in `Births.init_pre` that interpolated birth rates from a `pd.DataFrame` using a nonexistent `data_cols['cbr']` key. The branch was unreachable because `standardize_birth_data` always returns a `pd.Series` (or a rate/dist), and year interpolation is handled by `get_births` (`starsim/demographics.py`).

### Testing
- Added a CPU-normalized performance regression guard (`tests/test_baselines.py::test_performance_guard`, with baselines in `tests/perf_guard.yaml`) that fails if any of a small matrix of normalized timings — bare `init`/`run`, People-results off, a heterogeneous-`dt` sim, a births sim, and a `crn=False` sim — regresses by more than 1.5x. It guards distinct cost terms separately and uses min-of-repeats with CPU normalization to cancel machine speed. Because per-test wall-clock is unreliable under parallel contention, it is skipped under pytest-xdist and should be run serially (e.g. `pytest tests/test_baselines.py::test_performance_guard`) or refreshed via `tests/update_benchmarks.py`.
- Added `test_loop.py::test_plan_identity` (plus heterogeneous-fallback, insertion, and no-op-filtering variants) asserting the rebuilt integration plan matches the legacy object-sorted plan exactly; `test_timeline.py::test_timeline_container_independence`; `test_people.py::test_people_results_optout` and `test_cum_deaths_invariant`; `test_demographics.py::test_count_based_births` and `test_birth_data_cols`; and `test_randomness.py::test_no_jumping_under_crn_false` (asserting no RNG jumping occurs under `crn=False` and that it still does under `crn=True`).
...
- Under `ss.options.crn = False`, distributions no longer "jump" (reseed) between steps and draws; results remain deterministic for a fixed seed but differ from previous 3.5.x `crn=False` realizations. The default `crn=True` path is unchanged and bit-identical.
