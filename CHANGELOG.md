...
### Multiscale agents
- Agents now carry **two per-agent weights**: `scale` — the *result weight* (people represented in outputs), a float defaulting to `1.0` that becomes fractional (`1/ratio`) under splitting; and `epi_weight` — the *demographic & transmission weight* (whole people for vital dynamics and transmission participation), an **integer participation flag** defaulting to `1` (a whole body) that is set to `0` for non-participating fine sub-agents. They diverge only under multiscale; keeping them separate is what lets a rare outcome be resolved at fine scale without distorting transmission or vital dynamics.
- Two resolution primitives are provided (they may be mixed at any `ratio` within one simulation):
  - `sim.people.split(uids, ratio)` **partitions** each listed agent's outcome into `ratio`-times finer scale. All `ratio` agents (parent + `ratio-1` new siblings) get `scale = scale/ratio` as equal, independent outcome sub-draws; the **parent keeps its `epi_weight`** and stays one whole transmitting/reproducing body, while the **siblings get `epi_weight = 0`** and are non-participating sub-agents (tagged `people.fine`).
  - `sim.people.spawn_fine(parent_uids, n_events, ratio)` **resolves only the successes** of a rare branch: the disease model supplies `n_events[i]` in `[0, ratio]` (how many of each parent's `ratio` notional sub-people experienced the event), and only those `k` fine agents are materialized at `scale = scale/ratio`. The parent stays a whole body and sheds the delegated mass (`scale *= 1 - k/ratio`), conserving `sum(scale)`. This avoids creating agents for events that did not occur, so it is more economical than `split` when events are very rare.
- Both primitives copy all module states and draw fine-agent slots from a **dense, self-recycling pool** (lowest free slots above the reserved newborn range, reclaiming the slot of any fine agent that dies). Trajectories are independent and reproducible *within a run*; the allocation's largest slot stays O(n) regardless of `ratio`, sim length, or how many times a parent is re-resolved (there is no per-parent cohort cap). Fine-agent slots are not keyed to the parent, so they are not common across scenarios that change the split structure. A coarse parent may be re-resolved any number of times; re-splitting a fine sub-agent is unsupported and raises.
- Counting is **scale-weighted by default**: `Arr.count()` (and every auto-generated `n_<state>` result) counts agents by `scale`. `People.count(x)` is a new single entry point counting a boolean state/condition or a set of UIDs; `People.scale_flows(uids)` / `People.epi_flows(uids)` sum the result and demographic weights respectively.
...
- `Births` and `Pregnancy` operate on whole bodies (`epi_weight`): fine sub-agents are excluded from birth/conception selection, birth/pregnancy counts and rates are body-weighted, and newborns inherit the mother's `epi_weight`. Networks exclude fine sub-agents from transmission, so a split parent keeps transmitting. **`Deaths` is the exception**: fine sub-agents *do* face background death (it is a competing risk on the outcome they resolve — excluding them would let every one survive to the outcome and bias the resolved rate high, e.g. ~+18% for HPV cancer), and the death flow is **scale-weighted** (a fine agent's death removes its `1/ratio` of a person) so it matches the population that leaves `n_alive`.
- Fully backward compatible: with every `scale == epi_weight == 1` (the default), all results are identical to before. Result dtypes follow the weight that fills them: **scale-weighted** counts (every `n_<state>`, disease flows, `Deaths`, and the scale-weighted demographic counts) are stored as **float** to hold fractional weighted values; **body-weighted** vital-dynamics flows (`Births`, and `Pregnancy`'s `births`/`pregnancies`/`miscarriages`/`stillbirths`/`neonatal_deaths`/`maternal_deaths`/`n_preterm`/`n_very_preterm`) are integer counts and stay **int**. The `pop_scale` finalize multiply is now integer-preserving (`ss.scale_result`), so an integer result with an integral `pop_scale` (the default `1.0` included) stays integer rather than being promoted to float.
