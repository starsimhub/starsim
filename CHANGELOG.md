...
### Multiscale agents
- Agents now carry **two per-agent weights** (both default `1.0`): `scale` — the *result weight* (people represented in outputs); and `epi_weight` — the *demographic & transmission weight* (whole people for vital dynamics and transmission participation). They diverge only under multiscale; keeping them separate is what lets a rare outcome be resolved at fine scale without distorting transmission or vital dynamics.
...
- Fully backward compatible: with every `scale == epi_weight == 1` (the default), all results are identical to before. The `n_<state>` and flow/demographics count results are now stored as floats (to hold fractional weighted values), but equal the previous integer values for single-scale runs.
