...
- `sim.people.split(uids, ratio)` resolves each listed agent's outcome at `ratio`-times finer scale. All `ratio` agents (parent + `ratio-1` new siblings) get `scale = scale/ratio` as equal, independent outcome sub-draws; the **parent keeps its `epi_weight`** and stays one whole transmitting/reproducing body, while the **siblings get `epi_weight = 0`** and are non-participating sub-agents (tagged `people.fine`). It copies all module states and assigns CRN-safe slots derived from the parent (independent but reproducible trajectories). This is the only multiscale-specific call a disease model needs.
