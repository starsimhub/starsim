...
- Added `ss.SEIR`, which extends `ss.SIR` with a latent, non-infectious exposed state. Its `exposed` and `infected` states are the literal E and I compartments, and `ti_exposed` and `ti_infected` are the times of entering them.
- `ss.SIR.set_prognoses()` now splits into two methods that subclasses can override separately: `set_infection()` (make the agents infectious, which `ss.SEIR` delays by the latent period) and `set_progression()` (schedule recovery, death, and any other stages, relative to `ti_infected`).
...
- *Regression/bugfix*: `ssl.Measles` called `ss.SIR.set_prognoses()`, which set `infected = True` without clearing it, so exposed agents were simultaneously infectious: `dur_exp` had no effect on transmission and the model behaved as an SIR model. It also sampled `dur_inf` and `p_death` twice per infection. Both are fixed. As a result, `ssl.Measles.infected` (and hence `infectious`, `n_infected` and `prevalence`) now means the infectious compartment alone rather than exposed plus infectious, and `n_exposed` and `n_infected` are now disjoint. `ssl.Cholera` and `ssl.Ebola` were not affected by this, since neither inherited `ss.SIR.set_prognoses()`.
