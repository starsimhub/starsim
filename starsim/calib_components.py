    ...
    if isinstance(expected.index, pd.MultiIndex):
        ...
        for k in actual:
            for level, df in actual[k].groupby(non_t_levels):
                conformed.loc[(expected.index.get_level_values("t"), level), k] = (
                    np.interp(x=expected_t, xp=actual_t, fp=df)
                )
    else:
        for k in actual:
            conformed.loc[expected_t, k] = np.interp(
                x=expected_t, xp=actual_t, fp=expected[k]
            )
...
def linear_accum(expected, actual):
    ...
    if isinstance(expected.index, pd.MultiIndex):
        ...
        non_t_levels = [
            level for level in expected.index.names if level != "t" and level != "t1"
        ]
...
class CalibComponent(sc.prettyobj):
    ...
    def __init__(
        ...
        if isinstance(self.combine_reps, str) and hasattr(
            pd.core.groupby.DataFrameGroupBy, self.combine_reps
        ):
    ...
    def _validate_conform(self, conform):
        ...
        if not isinstance(conform, str) and not callable(conform):
            ...
            if conform_ == "NOT FOUND":
                ...
                raise ValueError(
                    f"The conform argument must be one of {avail}, not {conform}."
                )
        else:
            conform_ = conform
        return conform_
    ...
    def _combine_reps_nll(self, expected, actual, **kwargs):
        if self.combine_reps is None:
            ...
            # TODO: TEST!!!
            actual_combined = actual.groupby(expected.index.names).aggregate(
                func=self.combine_reps, **self.combine_kwargs
            )
            actual_combined["rand_seed"] = 0  # Fake the seed
            actual_combined = actual_combined.reset_index().set_index(
                "rand_seed"
            )  # Make it look like self.actual
    ...
    def eval(self, sim, **kwargs):
        ...
        if isinstance(sim, ss.MultiSim):
            ...
            if self.include_fn is not None and not self.include_fn(sim):
                sc.printv(
                    f"Warning: include_fn was specified for a single simulation, but will be ignored",
                    level=1,
                )
    ...
    def plot_facet_bootstrap(self, data, color, **kwargs):
        ...
        for bi in np.arange(n_boot):
            ...
            if self.combine_reps is None:
                ...
                actual = (
                    data.set_index("rand_seed")
                    .loc[use_seeds]
                    .groupby("t")
                    .aggregate(func=self.combine_reps, **self.combine_kwargs)
                )
...
class Binomial(CalibComponent):
    ...
    def get_p(df, x_col="x", n_col="n"):
        if "p" in df:
            ...
            p = (
                df[x_col] / df[n_col]
            )  # Switched to MLE x/n from previous "Bayesian" (Laplace +1, Jeffreys) before: (df[x_col]+1) / (df[n_col]+2)
        return p
    ...
    def compute_nll(self, expected, actual, **kwargs):
        ...
        for idx, rep in combined.iterrows():
            if "p" in rep:
                ...
                assert (
                    "n_e" in rep and "x_e" in rep
                ), "Expected columns n_e and x_e not found"
...
class DirichletMultinomial(CalibComponent):
    ...
    def plot(self, actual=None, **kwargs):
        ...
        actual = actual.reset_index()[["t", "calibrated", "rand_seed"] + x_vars].melt(
            id_vars=["t", "calibrated", "rand_seed"], var_name="var", value_name="x"
        )
        g = sns.FacetGrid(
            data=actual,
            col="t",
...
class GammaPoisson(CalibComponent):
    def __init__(self, *args, **kwargs):
        ...
        assert (
            self.expected["n"].dtype == int
        ), 'The expected must have an integer column named "n" for the total number of person-years'
        assert (
            self.expected["x"].dtype == int
        ), 'The expected must have an integer column named "x" for the total number of events'
        return
...
class Normal(CalibComponent):
    def __init__(
        self, name, expected, extract_fn, conform, weight=1, sigma2=None, **kwargs
    ):
    ...
    def plot_facet(self, data, color, **kwargs):
        ...
        for idx, row in data.iterrows():
            ...
            sigma2 = (
                self.sigma2 if self.sigma2 is not None else self.compute_var(e_x, a_x)
            )
            if isinstance(sigma2, (list, np.ndarray)):
                assert len(sigma2) == len(
                    self.expected
                ), "Length of sigma2 must match the number of timepoints"
    ...
    def plot_facet_bootstrap(self, data, color, **kwargs):
        t = data.iloc[0]["t"]
        expected = self.expected.loc[
            [t]
        ]  # Gracefully handle Series and DataFrame, if 't1' in index
        e_x = expected["x"].values.flatten()[
            0
        ]  # Due to possible presence of 't1' in the index
