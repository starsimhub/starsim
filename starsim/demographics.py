...
__all__ = ['Demographics', 'Births', 'Deaths', 'Pregnancy', 'Conception']
...
class Conception(ss.Condition):
    """ Conceptions are generated over sexual networks """
    ...
    def __init__(self, pars=None, *args, **kwargs):
        ...
        self.add_states(
            ...
            ss.State('ti_postpartum', int, ss.INT_NAN),  # Time postpartum ends
            ss.State('ti_dead', int, ss.INT_NAN),  # Maternal mortality
            ss.State('rel_sus', float, 1.0),  # Relative fecundibility
        )
        ...
        self.par_dists = ss.omergeleft(self.par_dists,
            sex_ratio = ss.bernoulli
        )
    ...
    def update_pre(self, sim):
        self.rel_sus[sim.people.age>=15] = 1
        self.rel_sus[(sim.people.age>=20) & (sim.people.age<25)] = 1.10
        self.rel_sus[(sim.people.age>=25) & (sim.people.age<28)] = 1.22
        self.rel_sus[(sim.people.age>=28) & (sim.people.age<31)] = 1.20
        self.rel_sus[(sim.people.age>=31) & (sim.people.age<34)] = 1.18
        self.rel_sus[(sim.people.age>=34) & (sim.people.age<37)] = 1.15
        self.rel_sus[(sim.people.age>=37) & (sim.people.age<40)] = 1.04
        self.rel_sus[(sim.people.age>=40) & (sim.people.age<45)] = 0.85
        self.rel_sus[(sim.people.age>=45) & (sim.people.age<50)] = 0.12
        self.rel_sus[(sim.people.age>=50)] = 0
        self.rel_sus[sim.people.male] = 0
    ...
    def init_results(self, sim):
        ...
        self.results += [
            ss.Result(self.name, 'prevalence', sim.npts, dtype=int, scale=True),
            ss.Result(self.name, 'pregnancies', sim.npts, dtype=int, scale=True),
            ss.Result(self.name, 'births', sim.npts, dtype=int, scale=True),
            ss.Result(self.name, 'cbr', sim.npts, dtype=int, scale=False),
        ]
    ...
    def _check_betas(self, sim):
        ...
        if isinstance(self.pars.beta, dict):
    ...
    def make_new_cases(self, sim):
        ...
        for nkey, net in sim.networks.items():
            ...
            # Adjust the relative susceptibility
            rel_sus = (people.female & people.alive & self.fecund & ~self.pregnant) * self.rel_sus
            trg = contacts.p2
            beta = nbetas[0]
            ...
            beta_per_dt = net.contacts.beta * beta * sim.dt
        ...
        if len(new_cases):
            ...
            self.make_embryos(sim, new_cases)
        else:
    ...
    def update_results(self, sim):
        ...
        res = self.results
        ti = sim.ti
        res.pregnancies[ti] = np.count_nonzero(self.ti_pregnant == ti)
        res.births[ti] = np.count_nonzero(self.ti_delivery == sim.ti)
        res.prevalence[ti] = res.n_pregnant[ti] / np.count_nonzero(sim.people.alive)
        return
