...
Summary
~~~~~~~
This version contains several major breaking changes. These include: module-specific timesteps and time-aware parameters (including a day/year ``unit`` flag for modules, and  ``ss.dur()`` and ``ss.rate()`` classes for parameters), as well as changes to module types and integration (removing ``ss.Connector`` and renaming ``update()`` and ``apply()`` methods to ``step()``).
