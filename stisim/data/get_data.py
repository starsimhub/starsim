'''
Download data needed for Starsim.

Typically, this is done automatically: on load, Starsim checks if the data are already
...
def get_data():
    ''' Download data '''
    sc.heading('Downloading Starsim data, please be patient...')
    T = sc.timer()
    ...
    T.toc('Done downloading data for Starsim')
...
def check_downloaded(verbose=1, check_version=True):
    ...
    for key,fn in loaders.files.items():
        ...
        if verbose>1:
            print(f'Starsim data: checking {fn}: {exists[key]}')
    ok = all(list(exists.values()))
    if not ok and verbose:
        print(f'Starsim data: at least one file missing: {exists}')
    elif ok and verbose>1:
        print('Starsim data: all files exist')
    ...
    if ok and check_version:
        ...
        if verbose:
            if not match and verbose:
                print(f'Starsim data: versions do not match ({metadata["version"]} != {data_version})')
            elif match and verbose>1:
                print(f'Starsim data: versions match ({data_version})')
        ok = ok and match
...
def remove_data(verbose=True, **kwargs):
    ...
    if verbose: sc.heading('Removing Starsim data files')
    for key,fn in loaders.files.items():
