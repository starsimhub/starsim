...
A callable alias behaves like a property: it takes the module as its only argument and is recomputed on each access, so it can be read but not written to. Unlike a string alias, it generates an automatic result, so `n_infected` still exists and counts E plus I. See [Aliases](modules_adding.qmd#aliases) for more.
