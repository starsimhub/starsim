...
`ss.prob` (and `ss.probperyear`, etc.) is a *number* — a time-aware scalar you can
multiply and convert between time periods. `ss.bernoulli` is a *distribution* you
*sample per agent*: `dist.filter(uids)` returns the UIDs for which the trial
succeeded. When you need to decide an outcome for each agent ("who gets infected, who
dies, who is vaccinated?"), use `ss.bernoulli`, not `ss.prob`. Use `ss.prob` when you
need the probability itself as a value — for example, a per-period probability that
will be scaled or fed into a distribution. See
[Distributions](advanced_distributions.qmd).
:::
