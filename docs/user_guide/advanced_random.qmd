...
For technical details and additional examples, please refer to the following publication: [D. J. Klein, R. G. Abeysuriya, R. M. Stuart, and C. C. Kerr, "Noise-free comparison of stochastic agent-based simulations using common random numbers." arXiv preprint arXiv:2409.02086 (2024).](https://arxiv.org/abs/2409.02086)

This guide builds on the [Distributions guide](advanced_distributions.qmd), so please familiarize yourself with that guide before proceeding.
...
To understand CRN, you first have to know a little about where random numbers come from. As discussed in the [Advanced Distributions guide](advanced_distributions.qmd), random numbers are used to make stochastic decisions within the model. Individuals realizations are generated from a pseudo-random number generator (PRNG) using a user-specified seed. The PRNG produces a sequence of numbers that appear random but are actually deterministic, meaning that the same seed will always produce the same sequence of numbers. Here is a simple demonstration using the `numpy` library:
...
**NOTE:** We have to specifically disable Starsim's CRN to observe the problem that all other agent-based modeling frameworks face. To do that in Starsim, we set the `crn` option to `False` in this example.
...
```{python}
# NOTE: For this example, we disable common random numbers (CRN)!
...
Comparing the two simulations, we can see that _different_ agents go fishing and catch fish in A versus B -- and in some timesteps _more_ agents catch fish in B than in A, even though all we did was ban agent 0 from fishing! The agents do not interact, so there should not have been a difference.
...
To see the benefit of CRN, we can simply repeat the above example, but this time restoring the `crn` option to its default value of `True`. This will ensure that the same random numbers are used for the exact same decisions in both simulations, allowing us to see the true impact of banning agent 0 from fishing:

```{python}
# Restore the crn option to True (this is the default, so you don't have to do this)
ss.options.crn = True
