...
6. **Use `ti_*` arrays for "who just changed state"** - e.g. agents infected on the previous step are `(disease.ti_infected == self.ti - 1).uids`, rather than tracking the previous set by hand
...
5. **Don't treat a boolean state as a list of UIDs** - `disease.infected[:]` is a boolean array and `np.array(disease.infected, dtype=int)` is just 0s and 1s; to get the agents, use `disease.infected.uids`
6. **Don't use `np.where(mask)[0]` to get agents** - it returns positions, not UIDs, which diverge from UIDs once any agent has died; use `mask.uids` instead
