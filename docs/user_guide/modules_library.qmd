...
`Measles` and `Ebola` show the most common pattern for a new disease: subclass `ss.SIR`, add extra states in `define_states()`, and extend `set_prognoses()` and `step_state()` to move agents between them. `Measles` adds a single exposed state; `Ebola` additionally makes severe and unburied agents more infectious, via `rel_trans`.
...
`CongenitalDisease` demonstrates the generic congenital framework built into `ss.Infection`. When transmission occurs to an unborn agent via `ss.PrenatalNet`, `set_congenital()` samples an outcome from the `birth_outcomes` distribution and schedules it for delivery time; `step_congenital()` then executes it. Lethal outcomes call `request_death()`, while non-lethal outcomes set a boolean state:
...
print('Fetal losses:', preg.miscarriages.sum() + preg.stillbirths.sum())
print('Congenital infections:', sim.diseases.congenitaldisease.congenital.sum())
```

To use the framework in your own disease, define `birth_outcome_keys` and `birth_outcomes` in your pars, add a matching `ti_<key>` state for each outcome (plus a boolean state for each non-lethal one), and call `self.step_congenital()` from `step_state()`.
