...
A callable alias behaves like a property: it's recomputed on each access, so it can be read but not written to, and it doesn't appear in `state_list`/`state_dict` or generate an `n_<name>` result. Note that a callable alias written as a lambda can't be saved with plain `pickle` (`sc.save()` and `ss.MultiSim` are fine); use a module-level function, or a property, if that matters to you.
