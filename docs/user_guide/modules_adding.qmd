...
```python
self.define_aliases(infectious='infected') # Another name for an existing state
self.define_aliases(asymptomatic=lambda self: self.infected & ~self.symptomatic) # Derived from other states
```
...
To replace an inherited state or alias rather than add to it, remove it first with `define_states(reset=...)`, which takes a name or list of names (or `True` for all of them):

```python
self.define_states(
    ss.BoolState('infectious', label='Infectious'), # Replaces the inherited `infectious` alias
    reset = ['infectious', 'ti_infected'], # Removing without redefining leaves the name undefined, so accessing it raises an AttributeError
)
```
