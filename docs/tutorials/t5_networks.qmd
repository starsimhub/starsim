...
## Sexual networks
Starsim contains a few basic sexual networks, including an `mf` network for modeling heterosexual partnerships and an `msm` network for men having sex with men. Modeling STIs is a straightforward extension of the SIR example we looked at previously. Here's an example modeling HIV:

```{python}
# Create the disease
hiv = ssl.diseases.HIV(
    beta = {'mf': [0.05, 0.025]},    # See notes below
)
...
# Make prenatal network
prenatal = ss.PrenatalNet()

# Make sim
sim = ss.Sim(pars=pars, diseases=hiv, networks=[mf, prenatal], demographics=[pregnancy, death], dur=10)
