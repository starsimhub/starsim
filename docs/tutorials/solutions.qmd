...
These solutions build on the `SIRS` class from the tutorial, so we define it again here:
...
class SIRS(ss.SIR):
    def __init__(self, pars=None, *args, **kwargs):
        super().__init__()
        self.define_pars(dur_imm=ss.lognorm_ex(mean=ss.years(10)))
    ...
    def set_progression(self, uids):
        ...
        recovering = uids[np.isfinite(self.ti_recovered[uids])]
...
**Q: Try different values of `dur_imm` in the SIRS model — how does it affect the epidemic curve?**

A: The shorter immunity is, the faster the susceptible pool is replenished, so transmission settles at a higher endemic level. With long-lasting immunity the model behaves much more like SIR: a single epidemic, followed by a slow return of susceptibles:

```{python}
sims = []
for dur_imm in [2, 5, 20]:
    sirs = SIRS(dur_imm=ss.lognorm_ex(mean=ss.years(dur_imm)))
    sims.append(ss.Sim(diseases=sirs, networks='random', dur=100, verbose=0, label=f'dur_imm={dur_imm}'))
msim = ss.parallel(sims)
msim.plot('sirs_n_infected')
```

### Question 2

**Q: Apply the same waning immunity to `ss.SEIR` to make an SEIRS model.**

A: The two new methods are identical to the ones in `SIRS`; only the base class changes. This works because `ss.SEIR` splits infection and progression the same way `ss.SIR` does, so overriding `set_progression()` picks up the latent period for free:
